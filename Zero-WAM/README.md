...
中文完整启动与排障指南见 [ROBOTWIN_EVAL_ZH.md](ROBOTWIN_EVAL_ZH.md)。
...
For the local checkout layout used here (`Zero-WAM` and `RoboTwin` as sibling
directories), the convenience launcher selects the `zerowam` server environment
and `RoboTwin` simulator environment automatically. It defaults to one trial:

```bash
bash evaluation/robotwin/run_single_eval.sh place_object_scale
```

Use environment variables to override the defaults, for example:

```bash
GPU_ID=0 TEST_NUM=100 SEED=0 \
SAVE_ROOT=/path/to/eval_results \
bash evaluation/robotwin/run_single_eval.sh place_object_scale
```

If one GPU cannot hold both the policy server and simulator, place them on
different devices with `SERVER_GPU_ID` and `CLIENT_GPU_ID`.
The convenience launcher enables VAE/text-encoder CPU offload by default for
48 GB-class GPUs; set `ZERO_WAM_ENABLE_OFFLOAD=0` on a larger dedicated GPU.

Run the human-video ICL protocol and save the predicted video stream:
...
Run the language-only control with the same checkpoint, seeds, and predicted
video visualization:
...
The two modes write to `results/icl` and `results/no_icl`, respectively.
Set `SAVE_IMAGINED_VIDEO=0` to skip VAE decoding and produce real-observation
videos only. The ICL launcher uses `ICL_CFG=5` and `TARGET_TEXT_CFG=-1`; the
no-ICL launcher uses language conditioning with `ICL_CFG=1` and
`TARGET_TEXT_CFG=1`.

For a non-benchmark smoke test, set `MAX_STEPS=16 TEST_NUM=1`. Leave
`MAX_STEPS` unset (or `0`) for official evaluation.

The lower-level two-terminal workflow is also available below.
