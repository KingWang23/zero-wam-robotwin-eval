...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        vae_device_name = getattr(
            job_config,
            'vae_device',
            'cpu' if self.enable_offload else 'cuda',
        )
        self.vae_device = (
            torch.device('cpu') if vae_device_name == 'cpu' else self.device
        )
        ...
        self.vae = load_vae(
            ...
            torch_device=self.vae_device,
        )
        logger.info(
            f"VAE device: {self.vae_device}; text encoder offload: "
            f"{self.enable_offload}"
        )
    ...
    def infer(self, obs):
        ...
        if self.use_icl_model:
            ...
            if compute_kv_cache:
                ...
                return dict()
            result = dict(action=self._infer_icl(obs))
            if obs.get("save_visualization", False):
                result["video"] = self.decode_one_video(
                    self.last_predicted_latents, "np"
                )[0]
            return result
