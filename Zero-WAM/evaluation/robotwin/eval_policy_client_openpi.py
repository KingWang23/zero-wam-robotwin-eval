...
def load_prompt_manifest(manifest_path):
    ...
    prompts = manifest.get("tasks", manifest)
    if not isinstance(prompts, dict):
        raise ValueError(f"Invalid prompt manifest: {manifest_path}")
    print(f"[PROMPT] replay manifest={manifest_path}", flush=True)
    return prompts


def select_episode_instruction(task_name, episode_id, episode_info, test_num,
                               instruction_type, instruction_seed,
                               prompt_manifest):
    task_prompts = prompt_manifest.get(task_name, {})
    prompt = task_prompts.get(str(episode_id))
    if prompt is not None:
        print(
            f"[PROMPT] replay task={task_name} episode={episode_id}: {prompt}",
            flush=True,
        )
        return prompt
    if prompt_manifest:
        raise KeyError(
            f"Prompt manifest has no entry for task={task_name!r}, "
            f"episode={episode_id}"
        )

    # RoboTwin's instruction templates use Python random, which setup_demo
    # does not seed. Isolate and seed both RNGs so future ICL/no-ICL runs pair.
...
def save_comparison_video(real_obs_list, imagined_video, action_history, save_path, fps=15):
    ...
    for i in range(n_frames):
        ...
        if imagined_video is not None and n_imagined:
            img_idx = 0 if n_real <= 1 else round(i * (n_imagined - 1) / (n_real - 1))
...
def load_icl_human_video_map(map_path):
    map_path = os.path.abspath(map_path)
    spec = importlib.util.spec_from_file_location(
        "robotwin_icl_human_videos", map_path
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Failed to import ICL human video map: {map_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    mapping = getattr(module, "ROBOTWIN_ICL_HUMAN_VIDEOS", None)
    if not isinstance(mapping, dict):
        raise RuntimeError(
            f"{map_path} must define dict ROBOTWIN_ICL_HUMAN_VIDEOS"
        )
    return mapping


def select_icl_video(mapping, task_name, seed, episode_idx):
    videos = mapping.get(task_name, [])
    if not videos:
        raise RuntimeError(f"No ICL human video configured for {task_name!r}")
    digest = hashlib.md5(task_name.encode("utf-8")).hexdigest()
    rng = random.Random(int(seed) + int(digest[:8], 16) + int(episode_idx))
...
def main(usr_args):
    ...
    icl_guidance_scale = usr_args["icl_guidance_scale"]
    icl_seed_value = usr_args.get("icl_seed")
    icl_seed = int(
        usr_args.get("seed", 0) if icl_seed_value is None else icl_seed_value
    )
    use_icl = bool(usr_args["use_icl"])
    save_imagined_video = bool(usr_args["save_imagined_video"])
    icl_video_mapping = (
        load_icl_human_video_map(usr_args["icl_human_video_map"])
        if use_icl
        else {}
    )
    icl_latent_root = usr_args["icl_latent_root"]
    episode_offset = int(usr_args.get("episode_offset") or 0)
    prompt_manifest = load_prompt_manifest(usr_args.get("prompt_manifest"))
    ...
    args["prompt_manifest"] = prompt_manifest
    args["use_icl"] = use_icl
    args["max_steps"] = int(usr_args["max_steps"])
    ...
    st_seed = usr_args.get("start_seed")
    # Match the Robotwin evaluation protocol used for the released checkpoint.
    st_seed = 100000 * (1 + seed) if st_seed is None else int(st_seed)
    suc_nums = []
    ...
    st_seed, suc_num = eval_policy(task_name,
                                   ...
                                   save_visualization=save_imagined_video,
...
def eval_policy(task_name,
    ...
    now_id = int(args.get("episode_offset", 0))
    succ_seed = 0
    suc_test_seed_list = []
    ...
    while succ_seed < test_num:
        ...
        TASK_ENV.setup_demo(now_ep_num=now_id, seed=now_seed, is_test=True, **args)
        if args["max_steps"] > 0:
            TASK_ENV.step_lim = min(TASK_ENV.step_lim, args["max_steps"])
        ...
        use_icl = bool(args["use_icl"])
        if use_icl:
            icl_video_path = select_icl_video(
                args["icl_video_mapping"],
                task_name,
                args["icl_seed"],
                now_id,
            )
            ...
            icl_video_path = ""
            icl_latent_path = ""
            print(f"[NO-ICL] task={task_name} episode={now_id}", flush=True)
        ret = model.infer(
            dict(
                reset=True,
                prompt=prompt,
...
def parse_args_and_config():
    ...
    parser.add_argument("--prompt_manifest", type=str, default=None)
    parser.add_argument("--test_num", type=int, default=100)
    parser.add_argument("--max_steps", type=int, default=0)
    parser.add_argument("--use_icl", type=int, choices=(0, 1), default=1)
    parser.add_argument(
        "--save_imagined_video", type=int, choices=(0, 1), default=0
    )
